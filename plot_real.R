...
for(i in 1:9){
  ...
  print(m[[i]],vp=vplayout(ceiling(i/3),loc[i]))
}



plothi<-function(maf){
  dat  = read.xlsx(file = "result_pom.xlsx",sheetIndex =maf*20)[2:5,]
  ...
  if(maf==0.05){
    a<-ggplot(df1,aes(x=beta, y=value, fill=factor(item,levels = levels(df1$item)[c(2,1,3)])))+
      ...
      theme(legend.key.width=unit(0.6,'cm'))+      # ggtitle(paste('MAF = ',maf))+
      labs(title=paste('MAF = ',maf))+
      theme(plot.title = element_text(hjust = 0.5))+
      theme(axis.title.x =element_text(size=10), axis.title.y=element_text(size=10),
            ,title =element_text (size=12))+
      scale_x_discrete(name=expression(beta), #x轴坐标名称
                       labels= c("ln1.2","ln1.3","ln1.4","ln1.5"))+ #离散的标签
