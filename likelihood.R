...
# likelihood.mod.grad    the gradient of the modfiy likelihood
...
p.likelihood.fisher = function(theta, data) {
  ...
  for (j in 1:J) {
    for (i in 1:n[j]) {
      x=data[[j]][i, ]
      bx = as.numeric(beta%*%x)
      bpx=as.numeric(beta.p%*%x)
      ph.j = phi(alpha[j + 1] - bx)
      ph.jm = phi(alpha[j] - bx)
      
      ##pro
      ph.j.p = phi(alpha.p[j + 1] - bpx)
      ph.jm.p = phi(alpha.p[j] - bpx)
      #beta
      beta.grad = -x * (1 - ph.j - ph.jm)
      a = x * sum(lambda * (1 - phi(alpha[2:(J + 1)] - bx) - phi(alpha[1:J] -
                                                                           bx)) * (phi(alpha[2:(J + 1)] - bx) - phi(alpha[1:J] - bx)))
      b = sum(lambda * (phi(alpha[2:(J + 1)] - bx) - phi(alpha[1:J] -bx)))
...
cp.estimate = function(y, G, X) {
  ...
  cov = matrix(c(cpro.var, va[J, 3 * J + K - 1], va[3 * J + K - 1, J], cmod.var), ncol = 2)
  a = cpro.var + (cp - cm) ^ 2
  b = cmod.var
  c = va[J, 3 * J + K - 1]
  ...
  #the test statistic of betap and betam
  cpro.p = 2 * (1 - pnorm(abs(cp / sqrt(cpro.var))))
  cmod.p = 2 * (1 - pnorm(abs(cm / sqrt(cmod.var))))

  # pm=c(cp,cm)
  # p1=1-pchisq(pm%*%ginv(cov)%*%pm,2)
  # f=function(w,cov,pm){
  #   cob = c(w, 1 - w)
  #   new.var = t(cob) %*% cov %*% cob
  #   new.statistic = (t(cob)%*%pm)/ sqrt(new.var)
  #   return(2 * (1 - pnorm(abs(new.statistic))))
  #  }
  # minw=optimize(f,c(0,1),cov=cov,pm=pm)
  # p2=minw$objective
  # c(cpro.p,cmod.p,new.p,p1,p2)
  
  # p0=ifelse(w>0.5,cpro.p,cmod.p)
  ...
  # 
  # mod.statistic = m / sqrt(ginv(-mod$hessian)[J, J])
  # ##p-value
  # pro.p = anova(pro.mod)[1, 3]
  # mod.p = 2 * (1 - pnorm(abs(mod.statistic)))
  out=c(cp, cm, new,cpro.p,cmod.p,new.p)
  names(out)=c("pro","mod","weight","cpT.p","cmT.p","wT.p")
  return(out)
  
}
##the function use R function
p.estimate = function(y, G, X) {
  J = length(table(y))
  N = length(y)
  ...
  sta = c(3:(3 + J - 2), rep(1, K + 1), 5:(5 + J - 2), 1:(1 + J - 2), rep(1, K + 1))
  ccdata = data.frame(y = y, g = G, x = X)
  data = df.to.list(ccdata)
  mod = maxLik(
    p.likelihood,
    grad = p.likelihood.grad,
    start = sta,
    constraints = list(ineqA = ui, ineqB = ci),
    data = data
  )
  co = mod$estimate
  ccdata = data.frame(y = y, G = G, X = X)
  B = p.likelihood.fisher(co, data = data)
  A = ginv(-mod$hessian / N)
  va =  A %*% B %*% A / N
  ...
  c = va[J, 3 * J + K - 1]
  v = (b - c) / (a + b - 2 * c)
  # w = ifelse(a + b - 2 * c > 0, min(max(0, v), 1), which.max(c(b, a)) - 1)
  # new = w * cp + (1 - w) * cm
  new = v * cp + (1 - v) * cm
  ...
  # mod.p = 2 * (1 - pnorm(abs(mod.statistic)))
  # 
  return(c(cp, cm, new, cpro.p, cmod.p, new.p))
}
